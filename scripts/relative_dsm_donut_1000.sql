...
CREATE TABLE IF NOT EXISTS jgg_centroid_relative_dsm_1000 (
    sgg_cd        text,
    tot_reg_cd    text,
    above_20_1000 numeric,
    below_20_1000 numeric,
    above_50_1000 numeric,
    below_50_1000 numeric
);
...
BEGIN
    ...
    LOOP
        ...
        INSERT INTO jgg_centroid_relative_dsm_1000 (
            sgg_cd,
            tot_reg_cd,
            above_20_1000,
            below_20_1000,
        ...
        WITH
            donut AS (
                ...
                SELECT
                    b.tot_reg_cd,
                    ST_Difference(b.geom_1030, b.geom_1000) AS geom
                FROM
            ...
            final_stats AS (
                ...
                SELECT
                    p.tot_reg_cd,
                    COUNT(*) FILTER (WHERE p.rast_value - sv.stnd > 20) * 1.0 / COUNT(*) AS above_20_1000,
                    COUNT(*) FILTER (WHERE p.rast_value - sv.stnd < -20) * 1.0 / COUNT(*) AS below_20_1000,
                    COUNT(*) FILTER (WHERE p.rast_value - sv.stnd > 50) * 1.0 / COUNT(*) AS above_50_1000,
                    COUNT(*) FILTER (WHERE p.rast_value - sv.stnd < -50) * 1.0 / COUNT(*) AS below_50_1000
                FROM
                    pixel_polygons p
