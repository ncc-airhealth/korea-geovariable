...
def run_point_calculation(
    self, calculator_class, year, coordinates, buffer_size=None, srid=4326
):
    """Helper function to run point calculation and handle exceptions."""
    try:
        if buffer_size:
            calc = calculator_class(buffer_size, year, coordinates, srid)
        else:
            calc = calculator_class(year, coordinates, srid)
        df = calc.calculate()
        return df_to_json(df)
    except Exception as e:
        self.update_state(
...
@celery_app.task(bind=True)
def calculate_point_bus_stop_count_task(
    self,
    coordinates: list[list[float]],
    buffer_size_value: int,
    year: int,
    srid: int = 4326,
):
    ...
    return run_point_calculation(
        self, CustomBusStopCountCalculator, year, coord_tuples, buffer_size, srid
    )


@celery_app.task(bind=True)
def calculate_point_hospital_count_task(
    self,
    coordinates: list[list[float]],
    buffer_size_value: int,
    year: int,
    ...
    return run_point_calculation(
        self, CustomHospitalCountCalculator, year, coord_tuples, buffer_size, srid
    )


@celery_app.task(bind=True)
def calculate_csv_file_task(
    self,
    file_id: str,
    calculator_type: str,
    buffer_size_value: int,
    ...
    if isinstance(supabase_url, str) and isinstance(supabase_key, str):
        supabase = create_client(supabase_url, supabase_key)
    else:
        raise ValueError("SUPABASE_URL or SUPABASE_ANON_KEY is not string")

    try:
        file_response = (
            supabase.from_("coordinate_files")
            .select("*")
            .eq("id", file_id)
            .single()
            .execute()
        ...
        batch_size = 100
        total_batches = len(coordinates) // batch_size + (
            1 if len(coordinates) % batch_size else 0
        )
        all_results = []
        ...
        for i in range(0, len(coordinates), batch_size):
            ...
            calc = calculator_class(buffer_size, year, batch_coords, srid)
        ...
        return {
            "error": str(e),
            "details": "Failed during CSV processing",
        }


@celery_app.task(bind=True)
def calculate_csv_file_task_direct_upload(
    self,
    file_data: bytes,
    calculator_type: str,
    buffer_size_value: int,
